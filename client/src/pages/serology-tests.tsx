...
export default function SerologyTests() {
  ...
  const { data: testsResponse, isLoading, error } = useQuery({
    queryKey: ['serology-tests'],
    queryFn: async () => {
      try {
        const response = await fetch('https://python-database-production.up.railway.app/api/serology-tests/', {
          headers: {
        ...
        console.error('Serology tests API error:', err);
        throw err;
      }
    },
    retry: 1,
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['serology-tests'] });
  ...
  const updateTestMutation = useMutation({
    mutationFn: async ({ name, data }: { name: string; data: any }) => {
      const response = await fetch(`https://python-database-production.up.railway.app/api/serology-tests/${encodeURIComponent(name)}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const deleteTestMutation = useMutation({
    mutationFn: async (name: string) => {
      console.log('Making request to:', `https://python-database-production.up.railway.app/api/serology-tests/${encodeURIComponent(name)}`, 'with method:', 'DELETE');
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <p>Failed to load serology tests. Please try again.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Serology Tests</h1>
          <p className="text-muted-foreground">Manage serology laboratory tests</p>
        </div>
        <Button 
          onClick={() => {
            resetForm();
