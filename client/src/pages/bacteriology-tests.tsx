...
export default function BacteriologyTests() {
  ...
  const { data: testsResponse, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        console.error('Bacteriology tests API error:', err);
        throw err;
      }
    },
    retry: 1,
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bacteriology-tests'] });
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <p>Failed to load bacteriology tests. Please try again.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Bacteriology Tests</h1>
          <p className="text-muted-foreground">Manage bacteriology laboratory tests</p>
        </div>
        <Button 
          onClick={() => {
            resetForm();
