...
export default function HaematologyTests() {
  ...
  const { data: testsResponse, isLoading, error } = useQuery({
    queryKey: ['haematology-tests'],
    queryFn: async () => {
      try {
        const response = await fetch('https://python-database-production.up.railway.app/api/haematology-tests/', {
          headers: {
        ...
        console.error('Haematology tests API error:', err);
        throw err;
      }
    },
    retry: 1,
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <p>Failed to load haematology tests. Please try again.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Haematology Tests</h1>
          <p className="text-muted-foreground">Manage haematology laboratory tests</p>
        </div>
        <Button 
          onClick={() => {
            resetForm();
