...
export default function BiochemistryTests() {
  ...
  const { data: testsResponse, isLoading, error } = useQuery({
    queryKey: ['biochemistry-tests'],
    queryFn: async () => {
      try {
        const response = await fetch('https://python-database-production.up.railway.app/api/biochemistry-tests/', {
          headers: {
        ...
        console.error('Biochemistry tests API error:', err);
        throw err;
      }
    },
    retry: 1,
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['biochemistry-tests'] });
  ...
  const updateTestMutation = useMutation({
    mutationFn: async ({ name, data }: { name: string; data: any }) => {
      const response = await fetch(`https://python-database-production.up.railway.app/api/biochemistry-tests/${encodeURIComponent(name)}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const deleteTestMutation = useMutation({
    mutationFn: async (name: string) => {
      console.log('Making request to:', `https://python-database-production.up.railway.app/api/biochemistry-tests/${encodeURIComponent(name)}`, 'with method:', 'DELETE');
  ...
  if (error) {
    return (
      <Card>
        ...
        <CardContent>
          <p>Failed to load biochemistry tests. Please try again.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Biochemistry Tests</h1>
          <p className="text-muted-foreground">Manage biochemistry laboratory tests</p>
        </div>
        <Button 
          onClick={() => {
            resetForm();
